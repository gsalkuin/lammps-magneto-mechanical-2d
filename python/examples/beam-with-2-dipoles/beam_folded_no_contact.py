...
"""
NOTE: Unlike the shape-locked lattice example, we load the new coordinates after the bond lengths have been computed.
"""
# fold the beam
msh_file
dump_txyz = np.loadtxt(msh_file+'-100.0MPa-folded.xyz', skiprows=2)
beam.V_into_xyz = dump_txyz[:,1:]

# "recenter"
beam.V_into_xyz = beam.V_into_xyz - np.mean(beam.V_into_xyz, axis=0)
...
filename = msh_file + '-' + str(Y_mod/1e7) + 'MPa-folded'
